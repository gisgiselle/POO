...
public class Batalla{
	...
	public Batalla(Heroe p1, Boss p2){
	int vida=p2.getHp();
			...
			switch(sc.nextInt()){
				...
				case 2:
					...
					for(int i=0;i<p1.getHabilidades().length;i++){
						System.out.print((i+1)+" "+p1.getHabilidades()[i].getNombreHabilidad()+" ["+p1.getHabilidades()[i].getPuntosEspeciales()+"] ");
					}
					System.out.println("");
				...
				case 3:
					...
					for(int i=0;i<p1.getMochila().length;i++){
						if(p1.getMochila()[i]!=null){
							System.out.print((i+1)+" "+p1.getMochila()[i].getNombreO()+" ["+p1.getMochila()[i].getPuntosO()+"] ");
						}else{
					...
					int opcionElegida=sc.nextInt();
						p1.atacar(p2,p1.getMochila()[opcionElegida-1],(opcionElegida-1));
					break;	
				default:
		...
		if(p2.getHp()<=0){
			...
			System.out.println(p2.getTipo()+" ya moriste");
		}
	}
